    ...
    if (need_save_) {
	  ...
	  string temp = "";
	    ...
	    if (image_name != temp) {
              temp = image_name;
              if (count > 0) {
                *(outfiles[label_name]) << count << std::endl;
                count = 0;
                for (int ii = 0; ii < scores.size(); ii++) {
                  vector<int> bbox = bboxes.at(ii);
                  float score = scores.at(ii);
              ...
              bboxes.clear();
              scores.clear();
              *(outfiles[label_name]) << image_name << std::endl;
            }
	  ...
	  *(outfiles[temp_label_name]) << count << std::endl;
          for (int ii = 0; ii < scores.size(); ii++) {
            vector<int> bbox = bboxes.at(ii);
            float score = scores.at(ii);
            *(outfiles[temp_label_name]) << bbox[0] << " " << bbox[1];
            *(outfiles[temp_label_name]) << " " << bbox[0] + bbox[2];
            *(outfiles[temp_label_name]) << " " << bbox[1] + bbox[3];
            *(outfiles[temp_label_name]) << " " << score;
            *(outfiles[temp_label_name]) << std::endl;
	  }
          for (int c = 0; c < num_classes_; ++c) {
            if (c == background_label_id_) {
              continue;
