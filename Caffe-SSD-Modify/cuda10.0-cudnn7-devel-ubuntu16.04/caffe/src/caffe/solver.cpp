...
void Solver<Dtype>::Step(int iters) {
  ...
  std::string log_directory = param_.snapshot_prefix().substr(0, param_.snapshot_prefix().find_last_of("/"));

  boost::filesystem::path file1("Train_Loss.txt");
  boost::filesystem::path trian_log_path = log_directory / file1;
  train_log_.open(trian_log_path.string().c_str(), std::ofstream::out);

  boost::filesystem::path file2("Test_Loss.txt");
  boost::filesystem::path test_log_path = log_directory / file2;
  test_log_.open(test_log_path.string().c_str(), std::ofstream::out); 
  ...
  while (iter_ < stop_iter) {
    ...
    if (display) {
      LOG_IF(INFO, Caffe::root_solver()) << "Iteration " << iter_
          << ", loss = " << smoothed_loss_;
      train_log_ << iter_ << ", " << smoothed_loss_ << std::endl;
...
void Solver<Dtype>::TestDetection(const int test_net_id) {
  ...
  for (int i = 0; i < all_true_pos.size(); ++i) {
    ...
    const string& output_name = test_net->blob_names()[output_blob_index];
    LOG(INFO) << "    Test net output #" << i << ": loss = " << loss << ", " << output_name << " = "
              << mAP << "\n\n";
    if (param_.test_compute_loss()) {
      test_log_ << iter_ << ", " << loss << ", " << mAP << std::endl;
    }
  }
}
